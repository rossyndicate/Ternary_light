...
Note that some chunks have been set to `eval = FALSE`, so they won't run unless
you manually run it. That way this book can run in reasonable timeframes on
computers that don't have a lot of memory or a fast download speed, and some
other chunks will run based on "caching." These chunks will be flagged
